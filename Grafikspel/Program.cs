...
Rectangle[] reds = {
    new Rectangle (100,500,36,36),
    new Rectangle (290,300,36,36),
    new Rectangle (800,400,36,36)
};
/// positioner på dom röda kvadraterna av dom 3

// Raylib.DrawRectangle(100,500,36,36, Color.RED);
// Raylib.DrawRectangle(290,300,36,36, Color.RED);
// Raylib.DrawRectangle(800,400,36,36, Color.RED);
...
Rectangle currentRed = reds[generator.Next(2)];


void tid()
{
    enemySpeed += 0.1f;   // Onda gubbe blir snabbare under varje sekund som man överlever  
}
...
int winScore = 6000; // du vinner när man når 8000 poäng
...
void reset() // när spelet börjar om så ska det här vara standard som man börjar på
{
    ...
    points = 0;
    enemySpeed = 1;
    currentRed = reds[generator.Next(2)]; /// Väljer mellan 0,1,2 Röda kvadrater
}
...
while (true) //While loop
{
    ...
    if (currentscene == "game") // när man spelar så gäller dessa knappar  A W S
    {
        ...
        if (Raylib.CheckCollisionRecs(character, currentRed))
        {
            currentscene = "gameover";
        }
        /// du dör om du nuddar den röda kvadraten 
    ...
    if (currentscene == "game")
    {
        ...
        Raylib.DrawRectangleRec(currentRed, Color.RED);
